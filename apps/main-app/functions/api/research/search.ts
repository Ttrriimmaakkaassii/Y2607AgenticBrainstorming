// Cloudflare Pages Function — server-side web search gateway for agents with
// webSearchEnabled. The browser never talks to Tavily directly and never
// sees TAVILY_API_KEY; it only ever calls this endpoint. Self-contained
// (like functions/api/chat.ts) — this directory is deliberately excluded
// from the Next.js/tsconfig build, so it doesn't share type imports with
// lib/ (see lib/web-search.ts for the client-side copy of these shapes).
